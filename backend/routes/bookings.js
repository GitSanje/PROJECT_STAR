...
router.get('/verify-payment',async (req, res) => {
  const { bookingId, sessionId,userID } = req.query;
  try {
    const booking = await Booking.findById(bookingId).populate('slotId');
    if (!booking || booking.paymentStatus !== 'pending') {
     
      return res.redirect(`http://localhost:3000/booking-failed?error=Booking not found or already processed`);
    }

    const slot = await Slot.findById(booking.slotId);
    console.log('====================================');
    console.log(slot, 'slot from verifypayment');
    console.log('====================================');
    ...
    //   return res.redirect(`http://localhost:3000/booking-failed?error=Slot is no longer available or locked by another user`);
    ...
    slot.bookingExpiresAt = new Date(Date.now() + booking.duration* 60 * 1000);
...
router.post('/', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `http://localhost:5002/api/bookings/verify-payment?bookingId=${booking._id}&sessionId={CHECKOUT_SESSION_ID}&userID=${req.user.id}`,
